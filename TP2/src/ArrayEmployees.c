...
int removeEmployee(eEmployee employees[],int len,eSector sectors[],int lenSector)
{
		...
		if(employees!=NULL && len>0 && sectors!= NULL && lenSector>0)
		 {
			if(pointOutIfThereIsSomethingLoaded(employees,len)==0)//CHECK IF THE USER PREVIOUSLY LOADED AN EMPLOYEE
			{
			...
			else
			{
				...
				do{
					printEmployees(employees, sectors,len,lenSector);//CHECK IF THE USER PREVIOUSLY LOADED AN EMPLOYEE
					optionRemMenu=removeEmployeeMenu();

					if(optionRemMenu==1)
					{
						printf("\n**** Remove Employee ****");
						utn_getNumber(&employeeId,"\n\nEnter id :","* ID NOT FOUND * ",0, 1100,2);
						index= searchEmployee(employeeId,employees,len);//SERCH EMPLOYEE
						...
						else//IF ID FOUND,SHOWS EMPLOYEE AND  REQUEST CONFIRMATION TO REMOVE
						{
							...
							printf("-----------------------------------------------------------------\n");
							printEmployee(employees[index],sectors,len,lenSector);
							printf("-----------------------------------------------------------------\n");
							printf("\n 1- CONFIRM");
							printf("\n 2- BACK TO MENU \n\n");
...
int modifyEmployee(eEmployee employees[],int len,eSector sectors[],int lenSector)
{
	...
	if(employees!=NULL && len>0 && sectors!= NULL && lenSector>0)
		...
		else
		{
			...
			printEmployees(employees,sectors,len,lenSector);
			utn_getNumber(&id,"Enter id :","Error.",0,1000,10);


			index=searchEmployee(id,employees,len);
			if(index==-1)//IF ID NOT FOUNF
			{
				printf("\n-----------------------------------------------------------------");
				printf("\n\n* ID NOT FOUND *\n");
...
void printEmployee(eEmployee employees,eSector sectors[],int len,int lenSector)
{
	...
	char status[10];
		...
		if(loadSectorDescription(employees.sector,sectors,lenSector,descSector))
		{
